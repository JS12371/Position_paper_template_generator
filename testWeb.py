...
def copy_footnotes(src_doc, dest_doc):
    footnote_map = {}
    for footnote in src_doc.footnotes.part.rels:
        footnote_id = src_doc.footnotes.part.rels[footnote].target_part
        footnote_content = footnote_id.element
        new_footnote = OxmlElement("w:footnote")
        new_footnote.set(qn("w:id"), footnote_id)
        new_footnote.append(footnote_content)
        dest_doc.footnotes.element.append(new_footnote)
        footnote_map[footnote_id] = new_footnote
...
def create_word_document(case_data, selected_arguments):  
    ...
    transferred_to_case = case_data['Transferred to Case #'] if 'Transferred to Case #' in case data else ['transferred to case not found']
    ...
    if issue[0] == 'Issue not found':
        try:
            issue = case_data['Issue Typ'].iloc[0].split(',') if 'Issue Typ' in case data else ['Issue not found']
        except:
            pass

    provider_numbers = ', '.join(case_data['Provider ID'].unique()) if 'Provider ID' in case data else 'Provider Numbers not found' 
